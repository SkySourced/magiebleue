...
impl ShaderProgram {
    ...
    pub fn attach_shader(&self, shader: &Shader) {
        unsafe { gl::AttachShader(self.glid, shader.glid) }
    }

    /// Links all attached shaders to this program
    pub fn link_program(&self) {
        unsafe { gl::LinkProgram(self.glid) };
    }
    ...
    pub fn link_success(&self) -> bool {
        let mut success = 0;
        unsafe {
            gl::GetProgramiv(self.glid, gl::LINK_STATUS, &mut success);
        }
    ...
    pub fn info_log(&self) -> String {
        ...
        unsafe { gl::GetProgramiv(self.glid, gl::INFO_LOG_LENGTH, &mut needed_len) };
...
impl Shader {
    ...
    pub fn compile(&self) {
        unsafe { gl::CompileShader(self.glid) };
    }

    /// Checks if the last compilation was successful
    pub fn compile_success(&self) -> bool {
        let mut compiled = 0;
        unsafe { gl::GetShaderiv(self.glid, gl::COMPILE_STATUS, &mut compiled) };
    ...
    pub fn info_log(&self) -> String {
        let mut needed_len = 0;
        unsafe { gl::GetShaderiv(self.glid, gl::INFO_LOG_LENGTH, &mut needed_len) };
        let mut v: Vec<u8> = Vec::with_capacity(needed_len.try_into().unwrap());
        let mut len_written = 0_i32;
        unsafe {
            gl::GetShaderInfoLog(
    ...
    pub fn delete(self) {
        unsafe { gl::DeleteShader(self.glid) };
    }
